...
import AddClinicDoctorModal from "../components/AddClinicDoctorModal"; // 👈 ایمپورت مودال
...
export default function ClinicDoctors() {
  ...
  // استیت کنترل مودال افزودن پزشک
  const [isModalOpen, setIsModalOpen] = useState(false);

  // تابع واکشی اطلاعات که با useCallback بهینه‌سازی شده تا قابل استفاده مجدد باشد
  const fetchClinicDoctors = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || "خطا در دریافت لیست پزشکان کلینیک",
      );
    } finally {
      setLoading(false);
    }
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredDoctors.map((doctor) => (
            ...
            >
              <div
                className={`absolute top-0 left-0 right-0 h-1.5 ${doctor.isActive ? "bg-green-500" : "bg-gray-300"}`}
              ></div>
              ...
              <div className="flex items-center gap-4 mb-5 border-b border-gray-100 pb-4 mt-2">
                ...
                <div className="flex-1 overflow-hidden">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${doctor.isActive ? "bg-green-50 text-green-600" : "bg-gray-100 text-gray-500"}`}
                    >
        ...
        <div className="bg-gray-50 p-10 rounded-2xl border border-dashed border-gray-200 text-center">
          ...
          <p className="text-gray-500 text-sm">
            {searchTerm
              ...
              : "شما هنوز هیچ پزشکی را به مرکز درمانی خود اضافه نکرده‌اید."}
          </p>
        </div>
      )}

      {/* 👈 اضافه کردن مودال به پایین صفحه */}
      <AddClinicDoctorModal
        isOpen={isModalOpen}
        onClose={() => setIsModalOpen(false)}
        onSuccess={fetchClinicDoctors} // فراخوانی تابع آپدیت لیست پس از موفقیت
