...
import api from "../../services/api"; // مسیرها رو نسبت به پوشه‌بندی خودت تنظیم کن
...
export default function LaboratoryProfile({
  ...
  const handleUploadSuccess =
    (field: keyof typeof labDocs) => (minioObjectName: string) => {
  ...
  const handleLabDocsSubmit = async () => {
    ...
    try {
      ...
      // 👈 اندپوینت اختصاصی آزمایشگاه
      const response = await api.post(
        "/laboratory/updatelaboratoryDocuments",
        payload,
      );
      ...
      if (response.data) {
        toast.success(
          response.data.message ||
            "مدارک با موفقیت تایید و در پرونده آزمایشگاه ثبت شدند.",
        );
  ...
  return (
    <div className="space-y-8 animate-in fade-in">
        ...
        <div className="bg-green-50 border border-green-200 rounded-2xl p-6 flex flex-col md:flex-row items-center gap-5 shadow-sm">
          ...
          <div className="text-center md:text-right">
            <h3 className="text-lg font-bold text-green-900 mb-1">
              پرونده مرکز آزمایشگاهی شما تایید و فعال است
            </h3>
            <p className="text-sm text-green-700/90 leading-relaxed">
              مدارک شما تایید شده است. اکنون به تمامی امکانات سامانه دسترسی
              دارید.
            </p>
          </div>
        </div>
        ...
        <div className="bg-amber-50 border border-amber-200 text-amber-800 rounded-2xl p-5 flex items-start gap-3 text-sm font-medium">
          ...
          <div>
            ...
            <p className="leading-relaxed text-amber-700/90">
              حساب کاربری آزمایشگاه شما غیرفعال است. جهت بررسی، باید تمامی مدارک
              خواسته‌شده در انتهای همین صفحه را بارگذاری نمایید.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          ...
          <div>
            ...
            <div className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-2.5 text-sm text-gray-700">
              {userProfile?.centerType === "Laboratory"
                ? "آزمایشگاه"
      ...
      {/* فرم بارگذاری مدارک (فقط در صورت غیرفعال بودن نمایش داده می‌شود) */}
      {!userProfile?.isActive && (
