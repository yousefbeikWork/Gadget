...
import {
  Search, Phone, MapPin, Activity, Loader2, 
  Building2, CalendarDays, User, ChevronRight, Calendar, AlignRight,
  X
} from "lucide-react";
...
export default function Clinics() {
  ...
  const [bookingStep, setBookingStep] = useState<'select-doctor' | 'select-slot'>('select-doctor');
  ...
  const fetchSlotsForDate = async (date: string) => {
    ...
    try {
      ...
      const response = await api.get(`/book/available-slots?doctorId=${bookingDoctor.id}&date=${date}`);
  ...
  const handleConfirmBooking = () => {
    ...
    toast.promise(
      bookingPromise,
      {
        ...
        error: (err) => err.response?.data?.message || 'خطا در رزرو نوبت. لطفاً دوباره تلاش کنید.'
      }
    );
  };
  ...
  return (
    <div className="flex-1 bg-white md:rounded-2xl shadow-lg p-6 md:p-8 overflow-y-auto custom-scrollbar relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gadget-dark">مراکز درمانی و کلینیک‌ها</h1>
          <p className="text-sm text-gray-500 mt-1">
            {userRole === "Patient" ? "کلینیک مورد نظر خود را انتخاب کرده و نوبت بگیرید" : "لیست مراکز درمانی ثبت شده در سامانه"}
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredClinics.map((clinic) => (
            <div key={clinic.id} className="bg-white border border-gray-200 rounded-2xl p-5 shadow-xs hover:shadow-md transition-shadow flex flex-col h-full group">
              <div className="flex items-center justify-between mb-5 border-b border-gray-100 pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className={`inline-block mt-1 px-2 py-0.5 rounded-md text-[10px] font-bold ${clinic.status === "فعال" ? "bg-green-50 text-green-600" : "bg-red-50 text-red-600"}`}>
                      {clinic.status}
                    </span>
                  </div>
                </div>
              ...
              <div className="mt-auto pt-4 border-t border-gray-100">
                ...
                {(userRole === "Patient" || userRole === "guest" || !userRole) && (
                  isLoggedIn ? (
                    <button
          ...
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
            ...
            <div className="bg-gadget-dark p-5 flex items-center justify-between text-white shrink-0">
              <div className="flex items-center gap-3">
                {bookingStep === 'select-slot' && (
                  <button onClick={() => setBookingStep('select-doctor')} className="hover:bg-white/20 p-1.5 rounded-lg transition-colors cursor-pointer">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-300">
                    {bookingStep === 'select-doctor' ? 'انتخاب پزشک' : `رزرو نوبت دکتر ${bookingDoctor?.lastName}`}
                  </p>
                </div>
              </div>
              <button onClick={closeBookingProcess} className="text-gray-300 hover:text-white bg-white/10 hover:bg-white/20 p-2 rounded-lg transition-colors cursor-pointer border-none outline-hidden">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
              ...
              {bookingStep === 'select-doctor' && (
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-4">پزشک مورد نظر را انتخاب کنید:</label>
                      ...
                      {bookingClinic.doctors.map((doc) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm font-bold text-gray-800">دکتر {doc.firstName} {doc.lastName}</p>
                            </div>
                          </div>
                          <CalendarDays size={18} className="text-gray-400 group-hover:text-gadget-light" />
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div className="text-center py-10 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                      <p className="text-sm text-gray-500 font-medium">این کلینیک هنوز پزشکی ثبت نکرده است.</p>
                    </div>
                  )}
                </div>
              )}
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            {availableSlots.map((slot, index) => {
                              ...
                              return (
                                ...
                                >
                                  <span dir="ltr">{slot.startTime}</span> <span className="text-[10px] font-normal opacity-80 mx-0.5">تا</span> <span dir="ltr">{slot.endTime}</span>
                                </button>
                              )
                            })}
                          </div>
